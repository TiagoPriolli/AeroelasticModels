function [coeff,localState,forces] = UVLM_FlatPlateLEV_NoSym(x,y,z,xdot,zdot,xc,yc,zc,xcdot,zcdot,...
...
%% Local Functions
    ...
    function [newXwakegrid,newYwakegrid,newZwakegrid] = UVLMWakeRollup
        ...
        for kk = 1:(NVRING+1)*(N+1)
            ...
            if all(circwakeLEV == 0)
                ...
                WakeUVWInducedLEV = [sum(UWInduced(:));...
                    sum(VWInduced(:));...
                    sum(WWInduced(:))];
            end
    ...
    function LESPValue = CalcLESP(circulation)
        LESPValue = circulation./(-X0dot*c*(acos(1-2*cp/c)+sin(acos(1-2*cp/c))));
    end
    ...
    function visualizeWakeDev
        ...
        LEVR = NLEVRingST+1;
        if any(LEVR>1)
            [XwakeLEV,YwakeLEV,ZwakeLEV] = FixLEVGrid_NoSym(XwakegridLEV,YwakegridLEV,ZwakegridLEV);
