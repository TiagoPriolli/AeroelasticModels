function [coeff,localState,forces] = UVLM_FlatPlateLEV(x,y,z,xdot,zdot,xc,yc,zc,xcdot,zcdot,...
...
StartLevShed = localState.StartLevShed;
...
if StartLevShed
    ...
    if any(levflag)
        ...
        circ = A\RHS;
        circ = reshape(circ,M,N);
        LESPIter(2,:) = CalcLESP(circ(1,:)+GammaLevIter(2,:));
        ...
        while any(levflag) %[Eq. 4.47]
            ...
            LESPIter(i,:) = CalcLESP(circ(1,:)+GammaLevIter(i,:));
            A0 = LESPIter(i,:);
...
if any(NLEVRingST > 0)
    ...
    % Delete LEV too far
    Lev2Far = all(abs(XwakegridLEV(NLEVRingST(1)+1,:) - Xgrid(M+1,:)) > c*10);
    if Lev2Far
        XwakegridLEV(NLEVRingST(1)+1,:) = zeros(1,N+1);
        YwakegridLEV(NLEVRingST(1)+1,:) = zeros(1,N+1);
        ZwakegridLEV(NLEVRingST(1)+1,:) = zeros(1,N+1);
        circwakeLEV(NLEVRingST(1),:) = zeros(1,N);
        NLEVRingST = NLEVRingST - 1;
    end
...
localState.StartLevShed = StartLevShed;
...
%% Local Functions
    ...
    function [RHS,uvw] = RightHandSide_FlatPlate
        ...
        for kk = 1:(N*M)
            ...
            if any(pseudoVortexflag)
                XpseudoGrid = [XwakegridLEV(1,:);Xgrid(1,:)];
                YpseudoGrid = [YwakegridLEV(1,:);Ygrid(1,:)];
                ZpseudoGrid = [ZwakegridLEV(1,:);Zgrid(1,:)];
    ...
    function LEVPosition
        ...
        if NLEVRingST(1) == 1
            UVW_LEV = zeros(3,N+1);
            for gg = 1:(N+1)
                XYZ = tib*[x(1,gg); y(1,gg); z(1,gg)]+[X0;Y0;Z0];
                XwakegridLEV(1,gg) = XYZ(1);
                YwakegridLEV(1,gg) = XYZ(2);
                ZwakegridLEV(1,gg) = XYZ(3);
                UVW_LEV(:,gg) = [-X0dot;-Y0dot;-Z0dot]...
                    + tib*([-Q*z(1,gg) + R*y(1,gg);-R*x(1,gg) + P*z(1,gg);-P*y(1,gg) + Q*x(1,gg)]...
                    + [xdot(1,gg);0;zdot(1,gg)]);
            end
            XwakegridLEV(2,:) = XwakegridLEV(1,:) + 0.5.*UVW_LEV(1,:).*dt;
            YwakegridLEV(2,:) = YwakegridLEV(1,:) + 0.5.*UVW_LEV(2,:).*dt;
            ZwakegridLEV(2,:) = ZwakegridLEV(1,:) + 0.5.*UVW_LEV(3,:).*dt;
    ...
    function visualizeWakeDev
        ...
        if any(pseudoVortexflag)
            for nn=1:numel(pseudoVortexflag)
                if pseudoVortexflag(nn)
                    Xpseudo = [XwakegridLEV(1,nn) XwakegridLEV(1,nn+1);Xgrid(1,nn) Xgrid(1,nn+1)];
                    Ypseudo = [YwakegridLEV(1,nn) YwakegridLEV(1,nn+1);Ygrid(1,nn) Ygrid(1,nn+1)];
                    Zpseudo = [ZwakegridLEV(1,nn) ZwakegridLEV(1,nn+1);Zgrid(1,nn) Zgrid(1,nn+1)];
                    mesh(Xpseudo,Ypseudo,Zpseudo,'FaceColor',[1 0 0],'EdgeColor',[1 0 0]);ylim([0 b]);zlim([-0.1 0.1]);
                end
            end
        end
        hold off
        ...
        surf(XLsp,YLsp,circ','EdgeColor','b','FaceColor','b');ylim([0 b]);zlim([-0.005 0.005])
        title('circulation')
