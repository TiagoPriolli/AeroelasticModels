...
offset_lastro = 0/1000;      %15/1000                                                    % deslocamento do centro de massa do lastro com rela??o ? linha de centro

Lw = 475e-3+25e-3;                                                                % envergadura da asa [m]
cw = ones(100,1).*45e-3*sqrt(pi/4);                                                                 % corda da asa [m]
cw(1:13) = 48.5e-3;
cw(14:16) = 51.5e-3*sqrt(pi/4);
cw(17:25) = 60.5e-3*sqrt(pi/4);
cw([42:45 82:85]) = 6.5e-3; 
tw = ones(100,1).*45e-3*sqrt(pi/4);                                                                  % espessura da asa [m]
tw(1:13) = 18.5e-3;
tw(14:16) = 51.5e-3*sqrt(pi/4);
tw(17:25) = 60.5e-3*sqrt(pi/4);
tw([35:38 75:78]) = 6.5e-3;
tw(90:100) = 12.8e-3;
ro = 2700;                                                                  % densidade do material da asa (aluminio 2024)
E = 70.1e9*0.2;                                                                 % m?dulo de Young do material
v = 0.3;                                                                   % coeficiente de Poisson do material
n_elem = 95+5; 
mw = ro.*(Lw/n_elem).*cw.*tw;                                                           % massa da asa dada pelo produto da densidade com o volume da asa
...
% Elementos centrais
for ii=1:n_elem
    m_e = mw(ii);                                                            % massa de cada elemento central
    Ixx_e = 1/12*m_e*(cw(ii).^2+tw(ii).^2);                                               % momento de in?rcia com rela??o ao eixo x
    Iyy_e = 1/12*m_e*(L_e.^2+tw(ii).^2);                                              % momento de in?rcia com rela??o ao eixo y
    Izz_e = 1/12*m_e*(L_e.^2+cw(ii).^2);                                              % momento de in?rcia com rela??o ao eixo z

    M_e_c(:,:,ii) = [m_e*eye(3) zeros(3,3)                                              % matriz de massa dos elementos centrais
            zeros(3,3) diag([Ixx_e Iyy_e Izz_e])];
end

% Elemento da raiz

m_e_r = mw(1)/2;                                                        % massa do elemento da ra?z
