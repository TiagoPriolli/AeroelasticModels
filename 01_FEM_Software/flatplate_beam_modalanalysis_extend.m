...
balance.Lw = 450e-3;                                                                % envergadura da asa [m]
balance.cw = 40e-3*sqrt(pi/4);                                                                 % corda da asa [m]
balance.tw = 40e-3*sqrt(pi/4);                                                                  % espessura da asa [m]
balance.ro = 2780;                                                                  % densidade do material da asa (aluminio 2024)
balance.E = 73.1e9;                                                                 % m?dulo de Young do material
balance.v = 0.33;                                                                   % coeficiente de Poisson do material

balance.mw = balance.ro.*balance.Lw.*balance.cw.*balance.tw;                                                           % massa da asa dada pelo produto da densidade com o volume da asa

balance.n_elem = 130;                                                                % n?mero de "elementos de viga", divis?o realizado ao longo da envergadura
...
m_e_r = balance.mw/balance.n_elem/2;                                                        % massa do elemento da ra?z
Ixx_e_r_cg = 1/12*m_e_r*(balance.cw^2+balance.tw^2);                                        % momento de in?rcia com rela??o ao eixo x
Iyy_e_r_cg = 1/12*m_e_r*((balance.L_e/2)^2+balance.tw^2);                                   % momento de in?rcia com rela??o ao eixo y
Izz_e_r_cg = 1/12*m_e_r*((balance.L_e/2)^2+balance.cw^2);                                   % momento de in?rcia com rela??o ao eixo z
...
%Elementos centrais
m_e = balance.mw/balance.n_elem;                                                            % massa de cada elemento central
Ixx_e = 1/12*m_e*(balance.cw^2+balance.tw^2);                                               % momento de in?rcia com rela??o ao eixo x
Iyy_e = 1/12*m_e*(balance.L_e^2+balance.tw^2);                                              % momento de in?rcia com rela??o ao eixo y
Izz_e = 1/12*m_e*(balance.L_e^2+balance.cw^2);                                              % momento de in?rcia com rela??o ao eixo z

balance.M_e_c = [m_e*eye(3) zeros(3,3)                                              % matriz de massa dos elementos centrais
zeros(3,3) diag([Ixx_e Iyy_e Izz_e])];
...
for i_node = 2:balance.n_nodes-1
    M(6*(i_node-1)+(1:6),6*(i_node-1)+(1:6)) = balance.M_e_c;                       % inclus?o dos demais blocos de massa, os quais s?o referente aos elementos do
end                                                                         % meio da asa.
...
K_e(3,3) = 12*by;
K_e(4,4) = t;
K_e(5,3) = -6*by*balance.L_e;
K_e(5,5) = 4*by*balance.L_e^2;
K_e(6,2) = 6*bz*balance.L_e;
K_e(6,6) = 4*bz*balance.L_e^2;

K_e(7,1)  = -a;
K_e(8,2)  = -12*bz;
K_e(8,6)  = -6*bz*balance.L_e;
K_e(9,3)  = -12*by;
K_e(9,5)  = 6*by*balance.L_e;
K_e(10,4) = -t;
K_e(11,3) = -6*by*balance.L_e;
K_e(11,5) = 2*by*balance.L_e^2;
K_e(12,2) = 6*bz*balance.L_e;
K_e(12,6) = 2*bz*balance.L_e^2;

K_e(7,7)   = a;
K_e(8,8)   = 12*bz;
K_e(9,9)   = 12*by;
K_e(10,10) = t;
K_e(11,9)  = 6*by*balance.L_e;
K_e(11,11) = 4*by*balance.L_e^2;
K_e(12,8)  = -6*bz*balance.L_e;
K_e(12,12) = 4*bz*balance.L_e^2;

balance.K_e = K_e+tril(K_e,-1).';                                                   % Matriz de rigidez do elemento
...
for i_elem=1:balance.n_elem
    K(6*(i_elem-1)+(1:12),6*(i_elem-1)+(1:12)) = ...
        K(6*(i_elem-1)+(1:12),6*(i_elem-1)+(1:12))+balance.K_e;
end
