...
offset_lastro = 0/1000;                                                     % deslocamento do centro de massa do lastro com rela??o ? linha de centro
n_eig = 30;                                                                 % n?mero de autovalores desejado, ou seja, n?mero de modos de vibra??o desejado

%% wing parameters
wing.Lw = 0.2e-3;%350e-3;                                                                % envergadura da asa [m]
wing.cw = 0.8e-3;%40e-3;                                                                 % corda da asa [m]
wing.tw = 0.8e-3;                                                                  % espessura da asa [m]
wing.ro = 5e11;                                                                  % densidade do material da asa (aluminio 2024)
...
wing.n_elem = 1;                                                                % n?mero de "elementos de viga", divis?o realizado ao longo da envergadura
...
m_e_t      = balance.mw(end)/2;                                                   % massa do elemento da ponta
Ixx_e_t_cg = 1/12*m_e_t*(balance.cw(end)^2+balance.tw(end)^2);                                        % momento de in?rcia com rela??o ao eixo x
Iyy_e_t_cg = 1/12*m_e_t*((balance.L_e/2)^2+balance.tw(end)^2);                                   % momento de in?rcia com rela??o ao eixo y
Izz_e_t_cg = 1/12*m_e_t*((balance.L_e/2)^2+balance.cw(end)^2);                                   % momento de in?rcia com rela??o ao eixo z
...
T_node_t_2_cg = [       eye(3)     -skew([-balance.L_e/4 0 0].')                    % operador de deslocamento do eixo de rota??o do centro de massa para a dist?ncia
                    zeros(3,3)                    eye(3)];                  % 'wing.L_e/4' localizado na extremidade do elemento.

balance.M_e_t_node = T_node_t_2_cg.'*M_e_t_cg*T_node_t_2_cg;                        % Matriz de massa com centro de rota??o deslocado para o n? localizado na
                                                                            % extremidade do elemento.

% Lastro

m_lastro   = 10;                                                       % massa do lastro
...
balance.M_e_t_node = balance.M_e_t_node + lastro*M_lastro_node;                             % Adicionando a massa do lastro ao elemento da ponta da asa. Vale ressaltar que o
...
M(6*(i_node-1)+(1:6),6*(i_node-1)+(1:6)) = balance.M_e_t_node;                      % inclus?o do ?ltimo bloco de massa, o qual ? referente ao elemento da ponta
...
modalstr.balance.Lw = balance.Lw;
modalstr.balance.cw = balance.cw;
modalstr.balance.tw = balance.tw;
modalstr.balance.mw = balance.mw;
...
%save(['flatplate_',num2str(1000*balance.Lw),'_',num2str(1000*balance.cw),'_',num2str(10000*balance.tw),'_ballast_',num2str(lastro),'_offset_',num2str(1000*offset_lastro),'_nelem_',num2str(wing.n_elem),'.mat'],'modalstr')
...
UD_y = [1;1]*modalstr.y;
UD_x = [-1;1]*(0*modalstr.y+modalstr.balance.cw(end)/2);
UD_z = 0*UD_y;
...
fat = 0.1*modalstr.balance.Lw/max(max(abs([D_x,D_y,D_z])));
